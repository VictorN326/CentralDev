...
export async function upVoteQuestions(params: QuestionVoteParams) {
  try {
    ...
    // Increment author's reputation for their contribution with +1 or -1
    await User.findByIdAndUpdate(userId, {
      $inc: { reputation: hasupVoted ? -1 : 1 },
    });

    // increment the author's reputation by  +10 / -10 for receiving an upvote or downvote
...
export async function deleteQuestion(params: DeleteQuestionParams) {
  try {
    ...
    ); // remove the question reference from the tag's questions array

    revalidatePath(path);
  } catch (error) {
    console.log("DEBUG: Error deleting question", error);
  }
}
...
export async function getRecommendedQuestions(params: RecommendedParams) {
  try {
    ...
    console.error("DEBUG: Error getting recommended questions:", error);
    throw error;
  }
}
